#!/usr/bin/env bash
#
# reset_bs2_light.sh — reseta dados do demo Bradesco EN SEM recriar Context Surface
#
# O que faz:
...
echo ""
echo "Bradesco EN — reset light"
echo "═════════════════════"

if ! grep -qE '^CTX_SURFACE_ID=.+' .env; then
  echo "ERRO: CTX_SURFACE_ID não está populado em .env."
  echo "Rode primeiro: bash scripts/start_bradesco_en_demo.sh"
  exit 1
fi

if ! grep -qE "^DEMO_DOMAIN=${DOMAIN}$" .env; then
  echo "ERRO: DEMO_DOMAIN ativo não é ${DOMAIN}. Rode start_bradesco_en_demo.sh primeiro."
  exit 1
fi
