#!/usr/bin/env bash
#
# reset_aiqfome_light.sh — reseta dados do demo aiqfome SEM recriar Context Surface
#
# O que faz:
...
#   6. Reseeda LangCache
#   7. Reseeda KYC perfil de fome 360 (documento + índice de fatias)
#
# Uso:
#   bash scripts/reset_aiqfome_light.sh
...
echo ""
echo "aiqfome — reset light"
echo "═════════════════════"

if ! grep -qE '^CTX_SURFACE_ID=.+' .env; then
  echo "ERRO: CTX_SURFACE_ID não está populado em .env."
  echo "Rode primeiro: bash scripts/start_aiqfome_demo.sh"
  exit 1
fi

if ! grep -qE "^DEMO_DOMAIN=${DOMAIN}$" .env; then
  echo "ERRO: DEMO_DOMAIN ativo não é ${DOMAIN}. Rode start_aiqfome_demo.sh primeiro."
  exit 1
fi
...
echo ""
echo "[7/7] Reseedando KYC perfil de fome 360..."
DEMO_DOMAIN="$DOMAIN" uv run python -m scripts.seed_kyc360_aiqfome
