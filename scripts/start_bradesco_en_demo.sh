#!/usr/bin/env bash
#
# start_bradesco_en_demo.sh — boot one-shot do demo Bradesco EN (ADA), do zero ao validado:
...
#
# Uso:
#   bash scripts/start_bradesco_en_demo.sh                # do zero, com tudo
#   bash scripts/start_bradesco_en_demo.sh --skip-setup   # só sobe (ambiente já setado)
#
# Reset light (re-seed sem recriar Context Surface):
#   bash scripts/reset_bradesco_en_light.sh
#
...
start_demo_core "bradesco_en" "bradesco-en-demo" "Bradesco EN" "$SKIP_SETUP"
