#!/usr/bin/env bash
#
# reset_bs2_light.sh — reseta dados do demo BS2 Pay SEM recriar Context Surface
#
# O que faz:
...
#   6. Reseeda LangCache
#   7. Reseeda KYC business-360 (momento do negócio + índice de fatias)
#
# Uso:
...
echo ""
echo "BS2 Pay — reset light"
echo "═════════════════════"

if ! grep -qE '^CTX_SURFACE_ID=.+' .env; then
  echo "ERRO: CTX_SURFACE_ID não está populado em .env."
  echo "Rode primeiro: bash scripts/start_bs2_demo.sh"
  exit 1
fi

if ! grep -qE "^DEMO_DOMAIN=${DOMAIN}$" .env; then
  echo "ERRO: DEMO_DOMAIN ativo não é ${DOMAIN}. Rode start_bs2_demo.sh primeiro."
  exit 1
fi
...
echo ""
echo "[7/7] Reseedando KYC business-360 (momento do negócio + índice de fatias)..."
DEMO_DOMAIN="$DOMAIN" uv run python -m scripts.seed_kyc360_bs2
