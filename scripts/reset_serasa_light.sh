#!/usr/bin/env bash
#
# reset_serasa_light.sh — reseta dados do demo Limpa Nome IA SEM recriar
# Context Surface. Útil pra iterar no seed/prompt/LTMs durante o demo, sem
# pagar o custo de recriar o Surface.
#
# O que faz:
...
DOMAIN="serasa_limpa_nome"

echo ""
echo "Limpa Nome IA (Serasa) — reset light"
echo "════════════════════════════════════"

if ! grep -qE '^CTX_SURFACE_ID=.+' .env; then
  echo "ERRO: CTX_SURFACE_ID não populado em .env."
  echo "Rode primeiro: bash scripts/start_serasa_demo.sh"
  exit 1
fi
