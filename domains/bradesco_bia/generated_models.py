"""Generated Context Surface models for the BIA domain."""
...
class Customer(ContextModel):
    """Customer entity for the BIA domain."""

    __redis_key_template__ = "bradesco_bia_customer:{customer_id}"
    ...
    segmento: str = ContextField(
        description="Segmento: varejo, exclusive, prime, private",
        index="tag",
    )
    ...
    score_interno: int = ContextField(
        description="Score interno Bradesco (0-1000)",
        index="numeric",
        sortable=True,
    )
...
class Account(ContextModel):
    """Account entity for the BIA domain."""

    __redis_key_template__ = "bradesco_bia_account:{account_id}"
...
class Card(ContextModel):
    """Card entity for the BIA domain."""

    __redis_key_template__ = "bradesco_bia_card:{card_id}"
    ...
    produto: str = ContextField(
        description="Produto: Elo Nanquim, Visa Infinite, Mastercard Black, etc.",
        index="text",
    )
...
class Transaction(ContextModel):
    """Transaction entity for the BIA domain."""

    __redis_key_template__ = "bradesco_bia_transaction:{txn_id}"
...
class BillingCycle(ContextModel):
    """BillingCycle entity for the BIA domain."""

    __redis_key_template__ = "bradesco_bia_billing_cycle:{cycle_id}"
...
class Investment(ContextModel):
    """Investment entity for the BIA domain."""

    __redis_key_template__ = "bradesco_bia_investment:{investment_id}"
...
class PixContact(ContextModel):
    """PixContact entity for the BIA domain."""

    __redis_key_template__ = "bradesco_bia_pix_contact:{contact_id}"
...
class Dispute(ContextModel):
    """Dispute entity for the BIA domain."""

    __redis_key_template__ = "bradesco_bia_dispute:{dispute_id}"
...
class FeatureStore(ContextModel):
    """FeatureStore entity for the BIA domain."""

    __redis_key_template__ = "bradesco_bia_features:{customer_id}"
...
class Policy(ContextModel):
    """Policy entity for the BIA domain."""

    __redis_key_template__ = "bradesco_bia_policy:{policy_id}"
    ...
    category: str = ContextField(
        description="Categoria: pix, contestacao, cartao, investimento, seguranca, prime, limites, lgpd",
        index="tag",
    )
